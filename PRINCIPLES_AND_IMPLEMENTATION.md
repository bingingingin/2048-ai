...
> 本文对照项目源码编写，解释程序实际做了什么、为什么这样实现，以及哪些能力还没有被验证。第 1–20 章以本地 C++ / Python 架构为主，算法基线为 `5ff8b0e`；2026-09-11 新增的浏览器独立运行与 GitHub Pages 发布见第 21 章。在线版编译同一份 C++ 源码，没有更换评估函数或搜索策略。
>
> 项目默认目标为 **65536**。目前已验证高位合并、自动控制和对局重放；已记录的三局完整测试分别达到 **16384、16384、8192**。支持目标方块的表示与搜索，不等于已经验证能够从空盘稳定达到目标。
>
> 作者补充的全部无撤回测试观察为：**快速模式的最大方块通常为 8192 或 16384；深思模式一般能稳定运行到 32768**。这是此前本地版的使用反馈，未附完整样本集合，不能换算成成功率或每局保证，也不直接代表新上线浏览器版的成绩。
...
21. [浏览器独立运行与网页发布](#21-浏览器独立运行与网页发布)
...
项目把“决定怎么走”“真正执行游戏”“控制自动运行”“显示状态”分开实现。下面是本地服务架构；在线版的 Web Worker 架构见第 21 章。
...
这既是一种恢复方法，也是一种一致性验证方法。本地格式为 `version: 1`；新增的浏览器格式为 `version: 2`。`verify_replay.py` 按版本分派到相应的独立 Python 校验器并拒绝未知版本。两种运行方式各自恢复自己的存档，不做跨格式迁移；跨 Python 版本的完全一致性也不应超出当前测试环境作保证。
...
这三个指定种子不是足以评估总体成功率的样本，也不是网页“深思”模式的完成批次。它们使用 10 ms、深度上限 8，快速预设则是深度上限 5，不应直接混为一组。

作者于 2026-09-11 补充了此前全部无撤回测试的总体观察：快速模式最大方块通常为 **8192 或 16384**；深思模式一般可以稳定到达 **32768**。此反馈未附总局数、种子列表和逐局记录，不能计算各模式到达率，也不表示数学上的必达保证。现有资料仍未提供从空盘达到 65536 的完整成功记录。来源、结构化观察与已有批次分别列在 [VALIDATION.md](VALIDATION.md) 和 [validation/user-observations.json](validation/user-observations.json)。
...
### 21.1 为什么选择 GitHub Pages

原本的网页依赖本机 Python HTTP 服务和动态库。GitHub Pages 只托管静态文件，不能运行这个后台进程。为了让别人打开链接就能玩，项目将同一份 `engine/solver.cpp` 编译为 WebAssembly，并把游戏执行器、控制器和存档搬到浏览器。

这种版本不需要服务器、数据库或账号，适合直接由公共仓库的 GitHub Actions 构建并发布到 [在线站点](https://bingingingin.github.io/2048-ai/)。本地版继续保留，适用于命令行批量测试和关闭网页后仍需持续计算的情况。

```mermaid
flowchart LR
    UI[网页交互与状态显示] --> CTRL[浏览器 Controller]
    CTRL -->|棋盘和搜索参数| WORKER[Web Worker]
    WORKER --> WASM[C++ 编译的 WebAssembly]
    WASM -->|方向和统计| CTRL
    CTRL --> GAME[JavaScript Game]
    GAME --> STORE[浏览器存档与 JSON 导出]
    GAME --> UI
```

### 21.2 构建与 WebAssembly 内存边界

`build_site.py` 使用 Emscripten **4.0.15**，以 `-O3 -std=c++17` 编译。生成的 `solver.mjs` 是 ES 模块，内部嵌入 WebAssembly 字节，避免额外的二进制加载路径配置。输出放在 `dist/`，由 Actions 作为 Pages artifact 上传，构建产物不进 Git。

原生搜索通过抛出 `Timeout` 提前退出深层递归，所以编译显式启用 `-fexceptions` 和 `DISABLE_EXCEPTION_CATCHING=0`。如果把异常处理裁掉，超时就无法正确回到迭代加深循环；对应的 WebAssembly 测试必须实际触发短预算搜索。

`wasm-client.mjs` 通过 `_malloc` 分配 16 字节输入等级、16 字节移动输出、32 字节方向评分和 32 字节统计数据。`_ai_choose` 和 `_ai_move` 是原来 C 接口的导出形式，没有重新用 JavaScript 编写搜索策略。

输入把实际方块值转换成等级；输出的四个 `double` 用小端 `DataView.getFloat64` 读取，四个 `uint64_t` 用 `getBigUint64` 读取后转换为普通数值。WebAssembly 内存增长可能更换 ArrayBuffer，因此每次调用后重新取得 `HEAPU8.buffer`，不长期复用旧视图。初始线性内存为 64 MiB，并允许增长。

工作线程在初始化查表完成后才发出 `ready`。加载失败和异常会显示给用户；如果工作线程已经失效，后续请求立即失败，避免等待永远不会返回的结果。

### 21.3 两种传输方式共用一个界面

源码 HTML 的 `2048-runtime` 元数据为 `local`，`app.js` 通过 `/api/` 与本地服务交互。构建静态站点时将该值改为 `browser`，界面动态加载 `browser/controller.mjs`，通过相同的 `request(path, data)` 约定读取状态和提交命令。

棋盘绘制、方向评分、模式选择、快捷键、滑动和导出按钮仍共用原来的界面。浏览器版只传棋盘与参数给 Worker，不传未来随机数；Worker 只做搜索，真正的移动、计分和落子在主线程 `Game` 中执行。所有资源路径改成相对路径，因此仓库站点的 `/2048-ai/` 前缀不会导致资源请求落到域名根目录。

### 21.4 控制器如何处理异步搜索

浏览器控制器以 `running`、单步计数和 `inFlight` 管理循环。一次只允许一个搜索请求；开始请求时复制棋盘并记录 `revision`，返回时再次检查修订号。暂停、手动移动、重开或改设置会增加修订号，之前的结果随后被丢弃。

暂停不需要强制中断正在执行的 WebAssembly；它立即改变控制状态，阻止该次搜索结果落到棋盘上。旧请求完成后才允许开始下一次搜索，避免同一引擎实例并发访问缓存和输入缓冲区。每轮通过定时器调度，计算本身在 Worker 内进行，主线程仍能响应用户操作。

出现目标方块时停止并归档；没有合法移动且启用重试时，新建一局、更新局数并继续。求解器异常会停止自动运行并显示错误。

### 21.5 浏览器随机数与可验证回放

`game.mjs` 独立实现标准移动和计分。初始种子由 `crypto.getRandomValues` 生成，后续随机数采用 32 位 Mulberry32，所有乘法用 `Math.imul` 保留低 32 位。整数抽样采用拒绝采样：

```text
limit = floor(2^32 / n) × n
不断取均匀 32 位输出 x，直到 x < limit
返回 x mod n
```

这样在“将一个 32 位输出映射到 n 个选项”这一环节避免直接取模的偏差。空位选择使用 `n = 空位数`；方块值使用 `n = 10`，其中 9 个结果生成 2，1 个生成 4。它仍是确定性伪随机数发生器，不是对每条有限序列都精确满足 9:1 的承诺。

回放包含 `version: 2`、`runtime: browser-wasm`、`rng: mulberry32-rejection-v1`，以及种子、目标、初始棋盘、每步方向与落子、最终棋盘、分数和终局状态。`Game.restore` 从种子开始完整重放并逐项比对，拒绝损坏的移动、落子或结果。

`browser_replay.py` 用 Python 重新实现相同的 32 位随机数算法，复用独立 Python 移动规则；`verify_replay.py` 识别版本后调用它。测试使用多个种子，将 JavaScript 生成的实际记录交给 Python 校验，同时检查恢复后继续运行的落子状态一致。浏览器和原本 Python Random 的种子语义不同，不能用同一个种子宣称两个平台进行了同一局比赛。

### 21.6 存档与生命周期

浏览器版的当前会话写入 `localStorage` 的 `2048-ai.browser.session.v2`，最近一份完整归档写入 `2048-ai.browser.archive.v2`。保存时机包括每 50 步、暂停、手动操作、设置变更、终局和页面隐藏。会话保留最多 100 条结果摘要，界面只显示最后 8 条；完整历史不会无限累积占用空间。

重开页面后校验存档并以暂停状态恢复。关闭标签页会结束计算，浏览器也可能限速后台标签页；它与本地 Python 后台进程的生命周期不同。存档由当前站点和浏览器管理，多个标签页共享同一个键，因此界面提示只在一个标签页运行。存储被禁用、配额不足或记录损坏时会显示提示，用户仍可游玩并导出当前对局。

### 21.7 发布、验证与许可

`pages.yml` 的构建步骤依次执行原生测试、浏览器控制器测试、固定 SDK 编译和 WebAssembly 测试；全部通过后，部署任务才获得 `pages: write` 与 `id-token: write` 权限发布。`build-info.json` 记录构建提交、C++ 源码 SHA-256 与编译器版本，便于核对在线版本。

WebAssembly 测试包括 83,521 种行布局、6,000 次整盘四方向对照、构造目标合并与死局、短预算异常退出，以及 200 步实际引擎运行和重放。这些检查说明构建后的程序能正确运行，不是达到 32768 或 65536 的整局成绩统计。

仓库原创部分采用 Apache-2.0，根目录附 `LICENSE` 与 `NOTICE`。上游改编部分继续保留 MIT 全文；构建脚本还从实际 SDK 复制 Emscripten、libc++、libc++abi、musl 和 compiler-rt 的许可文本到站点 `licenses/`。项目总许可不会替换第三方组件的许可与例外条款。
